...
const Project13 = () => {
  ...
  const randomText = (text) => {
    return Math.floor(text.length * Math.random());
  };
  const handleClick = () => {
    const getElement = document.querySelector(".project13__text");
    const valueInput = parseInt(input);
    let tempText = text.slice(0, valueInput);
    if (
      valueInput === "" ||
      valueInput < 0 ||
      valueInput === 1 ||
      ...
      getElement.innerHTML = `${text[randomText(text)]}`;
    } else {
      getElement.innerHTML = tempText.map((text) => {
        return (getElement.innerHTML = `${text} <br><br>`);
      });
    }
  };
  console.log(input);
  return (
    <div className="project13">
      <div className="project13__title">
        <h1>TIRED OF BORING LOREM IPSUM?</h1>
        <div className="project13__func">
          <h2>Paragraphs:</h2>
          <input
            value={input}
        ...
        <div className="project13__text"></div>
      </div>
    </div>
  );
};
